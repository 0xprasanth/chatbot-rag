...
class EmbeddingPipeline:
    ...
    def embed_chunks(self, chunks: List[Any]) -> np.ndarray:
        ...
        print(f"[INFO] Embedding chunks: {chunks}")
        texts = [chunk.page_content for chunk in chunks]

        print(f"[INFO] Generating embeddings for {len(texts)} chunks...")
        # embeddings = self.model.embed_documents(texts,show_progress_bar=True )

        embeddings = np.array(self.model.embed_documents(texts), dtype=np.float32)
...
if __name__ == "__main__":
    ...
    print(
        "[INFO] Example embedding:", embeddings[0][:20] if len(embeddings) > 0 else None
    )
